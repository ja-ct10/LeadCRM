  ...
  return (
    <div className="space-y-6">
          ...
          {(['Monthly', 'Quarterly', 'Annual'] as BillingView[]).map((v) => (
            <button key={v} onClick={() => setPricingView(v)}
              className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all ${
                pricingView === v
                  ? 'bg-white dark:bg-white dark:bg-slate-950 text-blue-600 shadow-sm border border-slate-200 dark:border-slate-700/50'
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {plans.map((plan) => (
          <div key={plan.id} className={`relative bg-white dark:bg-white dark:bg-slate-950 border rounded-2xl p-6 flex flex-col hover:shadow-md transition-shadow ${plan.isPopular ? 'border-blue-500 shadow-lg shadow-blue-500/10' : 'border-slate-200 dark:border-slate-700'}`}>
            {plan.isPopular && (
        ...
        {editingPlan && (
          <div className="fixed inset-0 bg-slate-900/40 dark:bg-slate-900/80 backdrop-blur-sm z-50 flex justify-end">
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }}
              className="bg-white dark:bg-white dark:bg-slate-950 w-full max-w-md h-full shadow-2xl flex flex-col border-l border-slate-200 dark:border-slate-700">
              ...
              <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/50 flex gap-3">
                <button onClick={() => setEditingPlan(null)} className="flex-1 px-4 py-2 bg-white dark:bg-white dark:bg-slate-950 border border-slate-300 dark:border-slate-600 text-slate-700 dark:text-slate-300 rounded-xl font-medium text-sm transition-colors">Cancel</button>
