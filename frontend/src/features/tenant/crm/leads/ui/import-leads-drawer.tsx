...
import { createLeadImport } from '@/shared/services/lead-imports.api';
import type { ImportRow } from '@/shared/services/lead-imports.api';
...
export function ImportLeadsDrawer({ isOpen, onClose, onImportComplete }: ImportLeadsDrawerProps): React.ReactElement {
  ...
  const handleImport = useCallback(async () => {
    ...
    try {
      const rows: ImportRow[] = csvRows.map((row, index) => ({
        rowNumber: index + 2,
        ...mapRow(row),
      }));

      const response = await createLeadImport({ fileName: file.name, rows });
        ...
        {importResult ? (
          <div className="flex-1 overflow-y-auto px-6 py-6">
            <ImportResultView
              ...
              onViewDetails={() => {
                handleClose();
                router.push(`/crm/leads/imports/${importResult.id}`);
              }}
              onClose={handleClose}
            />
          </div>
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-[13px] text-slate-500 dark:text-slate-400 mt-1">
          Upload your existing CSV file containing the leads you want to import.
        </p>
      </div>
