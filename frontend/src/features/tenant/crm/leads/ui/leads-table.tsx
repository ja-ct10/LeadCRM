...
import { getCRMStatusStyles, getConnectedDealsForLead, getConnectedDealsForOrg } from '@/lib/utils';
...
interface ClientTableProps {
  data: Lead[];
  viewMode: 'leads' | 'organizations';
  onEdit: (lead: Lead) => void;
  onView: (lead: Lead, tab?: string) => void;
  ...
  onQuickView?: (lead: Lead, tab?: string) => void;
        ...
        <table className="w-full border-collapse text-left">
          ...
          <tbody className="divide-y divide-gray-100 dark:divide-white/5">
            {filteredOrgs.map((org) => {
              ...
              const orgLeads = data.filter(c => !c.isArchived && ((c.organizationId && c.organizationId === org.id) || (c.companyName && org.name && c.companyName.toLowerCase().trim() === org.name.toLowerCase().trim())));
              ...
              return (
                <tr key={org.id} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                  ...
                  <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400">
                    <span className="font-medium text-slate-800 dark:text-slate-200">{orgLeads.length} Leads</span>
                  </td>
                  <td className="px-4 py-3">
                    {orgDeals.length > 0 ? (
                      <button 
  ...
  return (
    <div className="w-full overflow-x-auto">
      <table className="w-full border-collapse text-left">
        <thead>
          <tr className="border-b border-gray-200 dark:border-white/10 text-xs font-semibold text-slate-500 uppercase tracking-wider">
            <th className="px-4 py-3 min-w-25">Date</th>
            <th className="px-4 py-3 min-w-37.5">Company Name</th>
            <th className="px-4 py-3 min-w-37.5">Address</th>
            <th className="px-4 py-3 min-w-37.5">Lead Person</th>
            <th className="px-4 py-3 min-w-37.5">Lead Number</th>
            <th className="px-4 py-3 min-w-37.5">Email Address</th>
            <th className="px-4 py-3 min-w-37.5">Product Interest</th>
            <th className="px-4 py-3 min-w-30">Source</th>
            <th className="px-4 py-3 min-w-30">Agent</th>
            <th className="px-4 py-3 min-w-22.5">Deals</th>
            <th className="px-4 py-3 min-w-32.5 text-right">Value</th>
            <th className="px-4 py-3 min-w-22.5">Status</th>
            <th className="px-4 py-3 min-w-45">Update</th>
            <th className="px-4 py-3 text-right">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
          {data.map((lead) => {
            const leadDeals = getConnectedDealsForLead(lead, deals);
            const totalValue = leadDeals.reduce((sum, d) => sum + (d.value || 0), 0);

            return (
              <tr key={lead.id} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                <td className="px-4 py-3 text-sm text-slate-500 cursor-pointer" onClick={() => onView(lead)}>
                  {new Date(lead.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric'})}
                </td>
                <td className="px-4 py-3 cursor-pointer" onClick={() => onView(lead)}>
                  {lead.recordType === 'Organization' ? (
                    <div className="flex flex-col gap-0.5">
                      <span className="text-sm font-medium text-slate-800 dark:text-slate-200">{lead.companyName || '-'}</span>
                      {(lead.businessType || lead.orgWebsite) && (
                        <span className="text-[11px] text-slate-500 dark:text-slate-400">
                          {lead.businessType}{lead.businessType && lead.orgWebsite ? ' · ' : ''}{lead.orgWebsite}
                        </span>
                      )}
                    </div>
                  ) : (
                ...
                <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400">
                  {lead.address || [lead.streetAddress, lead.city, lead.province].filter(Boolean).join(', ') || '-'}
                </td>
                <td className="px-4 py-3 text-sm font-medium text-slate-900 dark:text-white cursor-pointer" onClick={() => onView(lead)}>
                  {lead.firstName ? `${lead.firstName} ${lead.lastName || ''}` : lead.leadPerson || '-'}
                </td>
                <td className="px-4 py-3">
                  <div className="flex flex-col gap-1 items-start">
                    {(lead.contactNumbers && lead.contactNumbers.length > 0) ? (
                      lead.contactNumbers.map(cn => (
                      ...
                      <span className="text-xs text-slate-600 dark:text-slate-400">{lead.phone || lead.mobileNumber || '-'}</span>
                    )}
                  </div>
                </td>
                <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400 break-all max-w-37.5">
                  {lead.email ? (
                ...
                <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400">
                  {(() => {
                    const agent = users.find(u => u.id === lead.assignedUserId);
                    return (
                      <div 
                ...
                <td className="px-4 py-3">
                  {leadDeals.length > 0 ? (
                    <button 
                      ...
                      onClick={(e) => { e.stopPropagation(); onView(lead, 'deals'); }}
                    >
              ...
              <td className="px-4 py-3">
                <span className={`px-2 py-0.5 rounded-full text-[10px] font-bold ${getCRMStatusStyles(lead.status)}`}>
                  {lead.status}
                </span>
              </td>
              <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400 max-w-37.5 truncate" title={lead.notes}>
                {lead.notes || '-'}
              </td>
              <td className="px-4 py-3 text-right">
                <div className="flex items-center justify-end gap-1">
                  <button onClick={(e) => { e.stopPropagation(); onQuickView && onQuickView(lead); }} className="p-1.5 text-slate-400 hover:text-indigo-600 transition-colors" title="Quick View">
                    <PanelRight size={16} />
                  </button>
                  <button onClick={(e) => { e.stopPropagation(); onEdit(lead); }} className="p-1.5 text-slate-400 hover:text-blue-600 transition-colors" title="Edit">
                    <Edit size={16} />
                  </button>
                  {lead.isArchived ? (
                    restoringId === lead.id ? (
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setRestoringId(lead.id); }}
                        className="p-1.5 text-amber-500 hover:text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-900/20 rounded-md transition-colors"
                        title="Restore Lead"
                      >
                        <ArchiveRestore size={16} />
                    ...
                    archivingId === lead.id ? (
                      ...
                      <button
                        onClick={(e) => { e.stopPropagation(); setArchivingId(lead.id); }}
      ...
      {selectedUserDrawer && (
        <UserProfileDrawer
          ...
          onSelectDeal={(deal) => {
            ...
            const targetLead = data.find(c => c.id === deal.leadId || deal.leadIds?.includes(c.id));
            if (targetLead) onView(targetLead);
          }}
        />
      )}
    </div>
