...
export function ManageColumnsDrawer({
  ...
  const handleDragEnd = useCallback((event: DragEndEvent) => {
    ...
    setLocalColumns((prev) => {
      ...
      if (searchQuery) {
        // When search is active, reorder only among visible (filtered) items
        // but preserve relative positions of non-matching items
        ...
        // Rebuild full list: replace matching items in their slots
        let matchIdx = 0;
      ...
      // No search filter — straightforward reorder
  ...
  // Focus trap: cycle tab within the drawer and auto-focus first element on open
  useEffect(() => {
    if (!isOpen) return;
