...
export interface TimelineEvent {
  ...
  actorName: string;
  entityId?: string;
  entityType?: 'contact' | 'organization' | 'deal' | 'task' | 'invoice';
  amount?: number;
}
...
export const CustomerJourneyTimeline: React.FC<CustomerJourneyTimelineProps> = ({
  ...
  // 1. Initial Contact Creation Event
  if (contact.createdAt) {
    events.push({
      id: `evt_created_${contact.id}`,
      category: 'CRM',
      eventType: 'lead_created',
      title: 'Lead Profile Created',
      description: `Lead intake registered via ${contact.leadSource || 'Inbound Inquiry'} into CRM system.`,
  ...
  const clientDeals = deals.filter(d => 
    ...
    (contactCompName !== '' && d.companyName && d.companyName.toLowerCase().trim() === contactCompName)
  );
  ...
  if (contact.updateStatus) {
    events.push({
      id: `evt_update_${contact.id}`,
      category: 'CRM',
      eventType: 'activity_log',
      title: 'Latest Interaction Logged',
      description: contact.updateStatus,
      timestamp: contact.lastUpdated || contact.createdAt,
