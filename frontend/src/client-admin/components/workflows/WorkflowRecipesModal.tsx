...
interface WorkflowRecipesModalProps {
  isOpen: boolean;
  onClose: () => void;
  addWorkflow: (data: Omit<Workflow, 'id' | 'tenantId' | 'executionCount'>) => void;
}
...
export const WorkflowRecipesModal = ({ isOpen, onClose, addWorkflow }: WorkflowRecipesModalProps) => {
  if (!isOpen) return null;

  const handleUseRecipe = (recipe: RecipeDefinition) => {
    addWorkflow({
      name: recipe.name,
          ...
          {RECIPES.map((recipe, idx) => (
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <span className={`text-[10px] px-2 py-0.5 rounded-full border ${categoryBadgeClass[recipe.category]}`}>
                  {recipe.category}
                </span>
                <button
