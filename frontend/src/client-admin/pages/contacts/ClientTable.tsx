...
interface ClientTableProps {
  ...
  onQuickViewOrg?: (org: Organization) => void;
              ...
              <tr key={org.id} className="hover:bg-slate-50 dark:hover:bg-slate-900 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      {org.website && <div className="text-xs text-blue-500">{org.website}</div>}
                    </div>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <div className="text-sm text-slate-700 dark:text-slate-300">{org.industry || '-'}</div>
                  <div className="text-xs text-slate-500">{org.size || '-'}</div>
                </td>
                ...
                <td className="px-4 py-3 text-sm font-medium text-slate-700 dark:text-slate-300">
                  <div className="flex items-center gap-1.5">
                    ...
                    {data.filter(c => c.organizationId === org.id).length}
                  </div>
                </td>
                <td className="px-4 py-3">
                  {(() => {
                    const orgContacts = data.filter(c => c.organizationId === org.id).map(c => c.id);
                    const orgDeals = deals.filter(d => orgContacts.includes(d.contactId || '') && !d.isArchived);
                    if (orgDeals.length === 0) return <span className="text-slate-400 text-xs">-</span>;
                    return (
                      <div className="flex flex-col gap-1">
                        <span className="text-xs font-medium text-slate-700 dark:text-slate-300">{orgDeals.length} Deal{orgDeals.length > 1 ? 's' : ''}</span>
                        <span className="text-[10px] text-slate-500">${orgDeals.reduce((sum, d) => sum + d.value, 0).toLocaleString()}</span>
                      </div>
                    );
                  })()}
                </td>
      ...
      <table className="w-full border-collapse text-left">
        <thead>
          <tr className="border-b border-gray-200 dark:border-white/10 text-xs font-semibold text-slate-500 uppercase tracking-wider">
            ...
            <th className="px-4 py-3 min-w-[100px]">Deals</th>
            <th className="px-4 py-3 min-w-[100px]">Status</th>
            <th className="px-4 py-3 min-w-[150px]">Update</th>
            <th className="px-4 py-3 text-right">Actions</th>
          </tr>
        </thead>
        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    {(contact.businessType || contact.orgWebsite) && (
                      <span className="text-[11px] text-slate-500 dark:text-slate-400">
                        {contact.businessType}{contact.businessType && contact.orgWebsite ? ' ΓÇó ' : ''}{contact.orgWebsite}
                      </span>
                    )}
                  </div>
                ) : (
              ...
              <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400">
                <div className="flex items-center gap-1.5">
                  <div className="w-5 h-5 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-[10px] font-medium text-slate-600 dark:text-slate-300">
                    {(users.find(u => u.id === contact.assignedUserId)?.firstName || '?').charAt(0)}
                  </div>
                  <span>{users.find(u => u.id === contact.assignedUserId) ? `${users.find(u => u.id === contact.assignedUserId)?.firstName} ${users.find(u => u.id === contact.assignedUserId)?.lastName}` : 'Unassigned'}</span>
                </div>
              </td>
              <td className="px-4 py-3">
                {(() => {
                  const contactDeals = deals.filter(d => d.contactId === contact.id && !d.isArchived);
                  if (contactDeals.length === 0) return <span className="text-slate-400 text-xs">-</span>;
                  return (
                    <button 
                      className="px-2 py-1 rounded-md text-xs font-medium bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-colors"
                      title="View Deals"
                      onClick={(e) => { e.stopPropagation(); /* Optional: handle clicking deals badge */ }}
                    >
