...
export default function ClientBillingPage() {
  ...
  const storageCost = extraStorage * 50; // Each extra GB costs G�50
  ...
  const invoices = [
    { id: 'INV-2026-001', date: 'Apr 01, 2026', amount: 'G�9,999.00', status: 'Paid', method: 'G��G��G��G�� 4242' },
    { id: 'INV-2026-002', date: 'Mar 01, 2026', amount: 'G�9,999.00', status: 'Paid', method: 'G��G��G��G�� 4242' },
    { id: 'INV-2026-003', date: 'Feb 01, 2026', amount: 'G�9,999.00', status: 'Paid', method: 'G��G��G��G�� 4242' },
  ];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div className="md:col-span-2 space-y-6">
              <div className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
                ...
                <div className="flex items-end gap-2 mb-6">
                  <span className="text-4xl font-extrabold text-slate-900 dark:text-white">G�9,999</span>
            ...
            <div className="space-y-6">
              <div className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm">
                ...
                <div className="text-sm text-slate-500 dark:text-slate-400 mb-4">Amount: G�9,999.00</div>
              ...
              <div className="bg-white dark:bg-[#0B1120] border border-slate-200 dark:border-slate-700 rounded-2xl p-6 shadow-sm space-y-5">
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-slate-500 dark:text-slate-400">Extra Users (+G�200/ea)</span>
                ...
                <div className="space-y-1.5">
                  <div className="flex justify-between items-center text-xs">
                    <span className="font-semibold text-slate-500 dark:text-slate-400">Extra Storage (+G�50/GB)</span>
                ...
                <div className="space-y-3.5 pt-2.5 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <label className="flex items-start gap-2.5 text-xs text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.checked) toast.success('Added Premium 24/7 SLA Support! G��');
                      }}
                      className="rounded border-slate-300 dark:border-slate-700 text-blue-600 focus:ring-blue-500 h-4 w-4 mt-0.5"
                    />
                    <div>
                      <div className="font-bold text-slate-800 dark:text-slate-200">Premium SLA Support (+G�1,500)</div>
                  ...
                  <label className="flex items-start gap-2.5 text-xs text-slate-600 dark:text-slate-400 cursor-pointer select-none">
                    ...
                    />
                    <div>
                      <div className="font-bold text-slate-800 dark:text-slate-200">Unlimited API Access (+G�2,500)</div>
                ...
                <div className="pt-3.5 border-t border-slate-150 dark:border-slate-800 space-y-1.5 text-xs">
                  <div className="flex justify-between text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                    ...
                    <span className="text-slate-750 dark:text-slate-200">G�9,999.00</span>
                  </div>
                  {extraUsers > 0 && (
                    <div className="flex justify-between text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                      <span>Extra Users ({extraUsers}):</span>
                      <span className="text-slate-755 dark:text-slate-200">+G�{userCost.toLocaleString()}</span>
                    </div>
                  )}
                  {extraStorage > 0 && (
                    <div className="flex justify-between text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                      <span>Extra Storage ({extraStorage} GB):</span>
                      <span className="text-slate-755 dark:text-slate-200">+G�{storageCost.toLocaleString()}</span>
                    </div>
                  )}
                  {premiumSupport && (
                    <div className="flex justify-between text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                      <span>Premium Support SLA:</span>
                      <span className="text-slate-755 dark:text-slate-200">+G�1,500.00</span>
                    </div>
                  )}
                  {advancedAPI && (
                    <div className="flex justify-between text-slate-500 dark:text-slate-400 font-mono text-[11px]">
                      <span>Enterprise API Access:</span>
                      <span className="text-slate-755 dark:text-slate-200">+G�2,500.00</span>
                    </div>
                  )}
                  {discountAmount > 0 && (
                    <div className="flex justify-between text-rose-500 dark:text-rose-400 font-bold font-mono text-[11px]">
                      ...
                      <span>-G�{discountAmount.toLocaleString()}</span>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-slate-50 dark:bg-slate-900/40 p-4 rounded-xl border border-slate-150 dark:border-slate-800 text-center sm:text-left">
                  ...
                  <div className="text-3xl font-extrabold text-slate-900 dark:text-white flex items-baseline justify-center sm:justify-start gap-1">
                    <span>G�{estimatedNextMonthlyAmount.toLocaleString()}</span>
                  ...
                  {frequency !== 'monthly' ? (
                    <p className="text-[10px] text-teal-600 dark:text-teal-400 font-bold mt-2 flex items-center gap-1 justify-center sm:justify-start bg-teal-50 ml-[-4px] mr-[-4px] dark:bg-teal-900/10 px-2 py-1.5 rounded-lg border border-teal-100/40 dark:border-teal-900/20">
                      ...
                      Dynamic Cycle: G�{(estimatedNextMonthlyAmount * (frequency === 'annual' ? 12 : 6)).toLocaleString()} billed every {frequency === 'annual' ? '12' : '6'} months.
                    </p>
                  ) : (
