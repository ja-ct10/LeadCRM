...
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip as RechartsTooltip, ResponsiveContainer, PieChart, Pie, Cell } from '../components/charts/ChartComponents';
    ...
    return (
      <div className="space-y-6 animate-in fade-in duration-300">
        <div className="flex items-center gap-4 mb-6">
          <button 
            onClick={() => setSelectedCampaignForReport(null)}
          ...
          <div>
            <h2 className="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">{selectedCampaignForReport.name} Report</h2>
        ...
        {(() => {
          const sent = selectedCampaignForReport.sentCount || 0;
          const bounceRt = 2.4; 
          const bounce = sent ? Math.round(sent * (bounceRt / 100)) : 0;
          const delivered = sent ? Math.round(sent * 0.976) : 0;
          const deliveredPct = sent ? 97.6 : 0;
          const opened = selectedCampaignForReport.openedCount || 0;
          const openedPct = sent ? Math.round((opened / sent) * 100) : 0;
          const clicked = selectedCampaignForReport.clickedCount || 0;
          ...
          return (
            <div className="space-y-4">
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
                ...
                <button
                  ...
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'sent'
                      ? 'bg-blue-50/50 dark:bg-blue-950/20 border-[#0A6EFF] shadow-[0_0_15px_rgba(10,110,255,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Total Sent</h3>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{sent.toLocaleString()}</div>
                  <div className="text-xs text-slate-400 dark:text-slate-500 mt-1.5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-slate-400 rounded-full"></span> 100% Sent
                  </div>
                </button>
                ...
                <button
                  ...
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'delivered'
                      ? 'bg-emerald-50/50 dark:bg-emerald-950/20 border-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center justify-between">
                    Delivered
                    <span className="text-[10px] bg-emerald-500/10 text-emerald-555 dark:text-emerald-400 px-1.5 py-0.5 rounded-full font-bold">New</span>
                  </h3>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{delivered.toLocaleString()}</div>
                  <div className="text-xs text-emerald-600 dark:text-emerald-400 mt-1.5 flex items-center gap-1 font-medium">
                    <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse"></span> {deliveredPct}% Rate
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setActiveMetricTab('opened')}
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'opened'
                      ? 'bg-blue-50/50 dark:bg-blue-950/20 border-blue-500 shadow-[0_0_15px_rgba(59,130,246,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Open Rate</h3>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{openedPct}%</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-blue-500 rounded-full"></span> {opened.toLocaleString()} opened
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setActiveMetricTab('clicked')}
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'clicked'
                      ? 'bg-blue-50/50 dark:bg-blue-950/20 border-teal-500 shadow-[0_0_15px_rgba(20,184,166,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Click Rate</h3>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{clickedPct}%</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-teal-500 rounded-full"></span> {clicked.toLocaleString()} clicked
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setActiveMetricTab('responded')}
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'responded'
                      ? 'bg-indigo-50/50 dark:bg-indigo-950/20 border-indigo-500 shadow-[0_0_15px_rgba(99,102,241,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1 flex items-center justify-between">
                    Responded
                    <span className="text-[10px] bg-indigo-500/10 text-indigo-500 dark:text-indigo-400 px-1.5 py-0.5 rounded-full font-bold">New</span>
                  </h3>
                  <div className="text-2xl font-black text-slate-900 dark:text-white">{responded.toLocaleString()}</div>
                  <div className="text-xs text-indigo-600 dark:text-indigo-400 mt-1.5 flex items-center gap-1 font-medium">
                    <span className="w-1.5 h-1.5 bg-indigo-500 rounded-full animate-pulse"></span> {respondedPct}% Rate
                  </div>
                </button>
                ...
                <button
                  type="button"
                  onClick={() => setActiveMetricTab('bounced')}
                  className={`text-left rounded-xl p-5 border transition-all ${
                    activeMetricTab === 'bounced'
                      ? 'bg-red-50/50 dark:bg-red-950/20 border-red-500 shadow-[0_0_15px_rgba(239,68,68,0.1)]'
                      : 'bg-white dark:bg-white/[0.02] border-gray-200 dark:border-white/[0.05] hover:border-gray-300 dark:hover:border-white/[0.12]'
                  }`}
                >
                  <h3 className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-1">Bounce Rate</h3>
                  <div className="text-2xl font-black text-red-500 dark:text-red-400">{bounceRt}%</div>
                  <div className="text-xs text-slate-500 dark:text-slate-400 mt-1.5 flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-red-500 rounded-full"></span> {bounce.toLocaleString()} bounced
                  </div>
                </button>
              </div>

              {/* Dynamic Interactive Metrics Explanation & Sub-dashboard Panel */}
              <div className="p-4 bg-slate-50 dark:bg-[#0c0f16] border border-gray-200 dark:border-white/[0.04] rounded-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-4 animate-in fade-in slide-in-from-top-1 duration-150">
                <div className="flex items-center gap-3">
                  <div className={`p-2.5 rounded-lg ${
                    ...
                    activeMetricTab === 'delivered' ? 'bg-emerald-500/10 text-emerald-555' :
                  ...
                  <div>
                    <h4 className="text-xs font-bold text-slate-700 dark:text-slate-300 uppercase tracking-widest">
                      {activeMetricTab === 'sent' && 'Sent Overview'}
                      {activeMetricTab === 'delivered' && 'In-box Delivery Report'}
                      {activeMetricTab === 'opened' && 'Recipient Open behavior'}
                      {activeMetricTab === 'clicked' && 'Click Link Performance'}
                      {activeMetricTab === 'responded' && 'Contact Response Breakdown'}
                      {activeMetricTab === 'bounced' && 'Bounces & Deliverability'}
                    </h4>
                    <p className="text-sm text-slate-600 dark:text-slate-400 mt-0.5">
                      {activeMetricTab === 'sent' && `The campaign broadcast successfully hit ${sent.toLocaleString()} active target contacts in the segment.`}
                      {activeMetricTab === 'delivered' && `High sender reputation yielded a success rate of ${deliveredPct}% with ${delivered.toLocaleString()} delivered emails.`}
                      {activeMetricTab === 'opened' && `${opened.toLocaleString()} individual contacts opened. Peak engagement occurred between 9:00 AM - 11:30 AM.`}
                      {activeMetricTab === 'clicked' && `${clicked.toLocaleString()} custom links were engaged (${clickedPct}% rate). The pricing sheet link led with 72% of total clicks.`}
                      {activeMetricTab === 'responded' && `${responded.toLocaleString()} contacts took action by writing a manual response or auto-initiating a direct CRM scheduling trigger.`}
                      {activeMetricTab === 'bounced' && `Soft bounce retry logs: 1.8% soft, 0.6% hard bounces. System automatically quarantined 3 inaccurate contact addresses.`}
                    </p>
                  </div>
                </div>
