            ...
            <div className="space-y-6 text-left animate-in fade-in duration-100">
              ...
              <form onSubmit={handleLogInteraction} className="bg-slate-50 dark:bg-white/[0.02] border border-gray-150 dark:border-white/[0.04] p-4 rounded-xl space-y-3">
                <div className="flex justify-between items-center pb-2 border-b border-gray-200 dark:border-white/5">
                  <h4 className="text-xs font-bold text-slate-800 dark:text-white uppercase tracking-wider">Quick log enterprise staff engagement</h4>
                ...
                <div className="flex gap-2">
                  <input 
                    type="text" 
                    required
                    placeholder={`e.g. Conducted technical consultation with ${selectedOrg.name} execs via web link...`}
                    value={logNotes}
          ...
          {activeTab === 'emails' && (
            <div className="space-y-4 text-left animate-in fade-in duration-100">
              <div className="bg-slate-50 dark:bg-white/[0.01] border border-gray-200 dark:border-white/[0.03] p-3 rounded-lg text-xs space-y-1">
                <span className="font-semibold block text-slate-400 uppercase text-[9px] tracking-wider">Select Corporate templates</span>
              ...
              <form onSubmit={handleSendEmail} className="space-y-3.5">
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase select-none mb-1">Corporate Dispatch Loop</label>
                  <input 
                    type="text" 
                    readOnly 
                    value={`[Broadcast to all (${selectedOrg.contacts.length}) registered key contacts of ${selectedOrg.name}]`} 
                    className="w-full bg-gray-100 dark:bg-white/[0.01] opacity-75 border border-gray-200 dark:border-white/[0.05] rounded-lg px-3 py-1.5 text-xs text-slate-500 font-bold"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Subject Line Header</label>
                  <input 
                    type="text" 
                    required
                ...
                <div>
                  <label className="block text-[10px] font-bold text-slate-400 uppercase mb-1">Broadcast Email Body Paragraphs</label>
                  <textarea 
                    rows={6}
                    required
                ...
                <div className="flex justify-end">
                  <button 
                    ...
                    className="bg-blue-600 hover:bg-blue-550 font-semibold text-white px-4 py-2 rounded-xl text-xs flex items-center gap-1.5 disabled:opacity-50"
                  >
                    <Send size={13} /> {isSendingEmail ? 'Broadcasting...' : 'Broadcast Email'}
                  </button>
                </div>
              </form>
            </div>
              ...
              <div className="bg-white dark:bg-[#040914] border border-gray-200 dark:border-white/[0.04] rounded-2xl max-w-sm mx-auto shadow-inner overflow-hidden">
                ...
                <div className="p-4 space-y-4 h-48 overflow-y-auto custom-scrollbar flex flex-col justify-end text-xs">
                  <div className="bg-slate-100 dark:bg-white/5 text-slate-800 dark:text-slate-300 p-2.5 rounded-2xl self-start max-w-[80%] text-left">
                    Hi! Setting up structured assessments for {selectedOrg.name}.
                  </div>
                  <div className="bg-blue-500 text-white p-2.5 rounded-2xl self-end max-w-[80%] text-left shadow-sm">
                    Acknowledged. Broadcast loop scheduled to mobile lines.
                  </div>
                </div>
                ...
                <form onSubmit={handleSendSms} className="p-3 border-t border-gray-200 dark:border-white/5 flex gap-2">
                  <input 
                    type="text"
                    required
                    placeholder="Broadcast SMS text to personnel..."
                    value={smsText}
