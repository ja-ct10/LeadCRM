...
const Page = dynamic(
  () => import("../../../src/features/tenant/crm/contacts/ui/contacts-page"),
  { ssr: false },
);
export default Page;
...
const PipelinePage = dynamic(
  () => import("../../../../src/features/tenant/crm/pipeline/ui/pipeline-page"),
  { ssr: false },
);
...
— the **same** value set as `Organization.customerType`. Only the _frontend_ `Contact` type
...
- Never write to `status` from a workflow, handoff, or import.
  Lifecycle position is a _separate_ system-owned axis. Keep the two apart and the requirement
...
Also: moving a deal to "Closed Lost" must use `moveDealStage()` (t

### DEV_OTP_BYPASS Env Var Was Never Wired Into sendLoginOtp

`DEV_OTP_BYPASS=true` and `DEV_SEED_EMAILS` were set in `.env` but never referenced anywhere in the codebase. The `sendLoginOtp` function always attempted a real Resend email — which fails on the free tier for any recipient other than the account owner. Fix: check `DEV_OTP_BYPASS === 'true'` at the top of `sendLoginOtp`, store `'000000'` as the OTP hash, and return early without sending email. This makes all seeder accounts testable locally without a verified Resend domain.he `/deals/:id/stage` endpoint), not `updateDeal()`. Calling `updateDeal` with only `{ lostReason }` saves the text but does not change the stage on the backend.
...
`crm-layout.tsx` was reported as dead code (nothing imports its _nav array_), but the file itself was imported by `app/(tenant)/layout.tsx` as the layout shell. Deleting it broke the build immediately. The lesson: "unused nav array inside a file" ≠ "unused file". Always run `tsc --noEmit` or grep for the file path before deleting, even when an audit says it's dead. The file's _internal_ duplication was the defect — the file itself had a live consumer.
