...
- [ ] 1. Enhance shared DataGrid component core features
  - [-] 1.1 Implement responsive column hiding via `useResponsiveColumns` hook
  ...
  - [-] 1.2 Add hidden columns indicator badge to DataGrid
    - Display count of auto-hidden columns when `hiddenCount > 0`
    - Render badge in the DataGrid toolbar area
    - _Requirements: 7.7_

  - [-] 1.3 Implement tooltip on truncated cells (clip mode)
  ...
  - [~] 1.4 Implement column width persistence via `onColumnWidthChange` prop
  ...
  - [~] 1.5 Implement enhanced empty state support with `EmptyStateConfig` prop
  ...
  - [~] 1.6 Add ARIA grid pattern and keyboard navigation
  ...
  - [-] 1.7 Implement view mode toggle (`wrap` | `clip`) in DataGrid
  ...
  - [-] 1.8 Implement default null/empty cell rendering as em-dash
...
- [~] 2. Checkpoint — Ensure all DataGrid core tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 3. Enhance DataGrid column reorder and selection
  - [~] 3.1 Enhance `useColumnDragReorder` hook for sequential order values
  ...
  - [~] 3.2 Implement reorder persistence with optimistic rollback
  ...
  - [~] 3.3 Enhance `useBulkSelection` hook with 100-record cap
...
- [ ] 4. Enhance ManageColumnsDrawer and column header menu
  - [~] 4.1 Enhance ManageColumnsDrawer search and grouping
  ...
  - [~] 4.2 Implement ManageColumnsDrawer save with retry and unsaved-changes guard
...
- [ ] 5. Visual consistency and styling standardization
  - [~] 5.1 Standardize DataGrid header and row styling across all modules
  ...
  - [~] 5.2 Implement colgroup-based column width sharing
  ...
  - [~] 5.3 Implement sticky/frozen primary column with inset shadow
  ...
  - [~] 5.4 Standardize cell renderers: dates, links, status badges, avatars
...
- [~] 6. Checkpoint — Ensure all shared DataGrid tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Implement Deals module DataGrid migration
  - [~] 7.1 Create `deals-data-grid.tsx` component
  ...
  - [~] 7.2 Wire Deals DataGrid to preference hooks and ManageColumnsDrawer
  ...
  - [~] 7.3 Integrate Deals DataGrid with existing page (table/kanban view switch)
...
- [ ] 8. Implement search, filter, sort, and pagination consistency
  - [~] 8.1 Standardize ModuleWorkspace toolbar controls order
  ...
  - [~] 8.2 Implement consistent pagination across Contacts and Accounts
  ...
  - [~] 8.3 Implement search with 300ms debounce and case-insensitive matching
    - Debounce at 300ms, reset pagination to page 1
    - Match against module's searchable fields (name, email, phone)
    - Case-insensitive substring matching
    - _Requirements: 8.6_

  - [~] 8.4 Implement sort and filter persistence with error handling
...
- [~] 9. Checkpoint — Ensure all module integration tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 10. Create shared EntityCombobox component
  - [~] 10.1 Implement `EntityCombobox` reusable component
  ...
  - [~] 10.2 Implement Account combobox for Deal form
  ...
  - [~] 10.3 Implement Contact multi-select for Deal form
...
- [ ] 11. Audit and reconcile Create/Edit forms
  - [~] 11.1 Audit and fix Leads Create/Edit form against `CreateContactSchema`/`UpdateContactSchema`
  ...
  - [~] 11.2 Audit and fix Contacts Create/Edit form against `CreateContactSchema`/`UpdateContactSchema`
    - Same reconciliation as Leads
    - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5_

  - [~] 11.3 Audit and fix Accounts Create/Edit form against `CreateCompanySchema`/`UpdateCompanySchema`
    - Same reconciliation pattern
    - _Requirements: 9.1, 9.2, 9.3, 9.4, 9.5_

  - [~] 11.4 Implement Deals Create form with all `CreateDealSchema` fields
  ...
  - [~] 11.5 Implement Deals Edit form with `UpdateDealSchema` fields
  ...
  - [~] 11.6 Implement form UX: scroll-to-error, inline validation, currency input
...
- [~] 12. Checkpoint — Ensure all form tests pass
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 13. Table-to-form data consistency and backend verification
  - [~] 13.1 Verify Column Registry ↔ API response DTO field mapping per module
  ...
  - [~] 13.2 Verify form field names match backend DTO field names
    - Ensure form submission payloads accepted without client-side transformation
    - Remove phantom fields, add missing fields
    - _Requirements: 12.3, 12.4, 12.5_

  - [~] 13.3 Verify shared types in `shared/src/types/` are referenced by both frontend and backend
    - Ensure compile-time detection of field name mismatches
    - _Requirements: 12.6_

  - [~] 13.4 Verify backend `reconcileWithRegistry` handles stale/new column IDs
  ...
  - [~] 13.5 Verify all table-preferences API routes are implemented
  ...
  - [~] 13.6 Verify tenant isolation and RBAC on all preference endpoints
...
- [ ] 14. Performance optimization and final wiring
  - [~] 14.1 Implement memoization strategy for DataGrid
  ...
  - [~] 14.2 Ensure pagination limits DOM to max 50 rows
    - All modules enforce max 50 records per page
    - _Requirements: 15.5_

  - [~] 14.3 Implement column header menu keyboard accessibility
  ...
  - [~] 14.4 Wire all five modules to use standardized DataGrid with consistent props
