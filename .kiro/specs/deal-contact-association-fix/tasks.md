...
> **STATUS: DONE & VERIFIED (2026-09-02).** P1. All tasks complete.
> syncContactAssociations → ContactDeal/Contact; syncLeadAssociations → LeadDeal/Lead;
> updateDeal wired for both; duplicateDeal copies ContactDeal. Property tests added.
> Smoke: create deal → update contactIds → ContactDeal set-equality → no LeadDeal written → PASS.
> Backend tsc clean; 163/163 tests.

- [x] 1. Repository: fix + split the sync functions
...
- [x] 2. Service: wire both syncs in `updateDeal`; fix `duplicateDeal`
