...
import * as service from './deal-imports.service';
import { ListDealImportsQuerySchema, ListDealImportResultsQuerySchema } from './deal-imports.dto';
...
export async function listImports(req: Request, res: Response, next: NextFunction) {
  try {
    const query = ListDealImportsQuerySchema.parse(req.query);
...
export async function getImportResults(req: Request, res: Response, next: NextFunction) {
  try {
    const importId = String(req.params.importId);
    const query = ListDealImportResultsQuerySchema.parse(req.query);
